...
async function exportFast1080(){
  ...
  const button=$('#exportButton'),lengthSelect=$('#exportLength'),fpsSelect=$('#exportFps');button.disabled=true;lengthSelect.disabled=true;fpsSelect.disabled=true;button.classList.add('recording');$('#exportNote').classList.remove('error');$('#exportNote').textContent='Rendering constant-frame-rate 1080p video…';
  ...
  try{
    ...
    await audioEncoder.flush();muxer.finalize();downloadBlob(new Blob([target.buffer],{type:'video/mp4'}),'mp4');$('#exportNote').textContent=`Constant ${fps} FPS MP4 exported in ${width} × ${height} with protected audio headroom.`;button.innerHTML='<span>✓</span> EXPORTED';setTimeout(()=>button.innerHTML='<span>↓</span> EXPORT 1080P MP4',1600);
  }catch(error){console.error(error);$('#exportNote').textContent=error.message||'Fast export failed on this device.';$('#exportNote').classList.add('error');button.innerHTML='<span>!</span> EXPORT FAILED';setTimeout(()=>button.innerHTML='<span>↓</span> EXPORT 1080P MP4',1800)}finally{if(videoEncoder?.state!=='closed')videoEncoder?.close();if(audioEncoder?.state!=='closed')audioEncoder?.close();offlineRendering=false;canvas.width=oldWidth;canvas.height=oldHeight;button.disabled=false;lengthSelect.disabled=false;fpsSelect.disabled=false;button.classList.remove('recording')}
}
