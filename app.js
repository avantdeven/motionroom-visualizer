...
const state = {
  playing: false, reference: null, influence: .45, motion: .68, seed: 12,
...
async function exportFast1080(){
  ...
  try{
    ...
    const selected=lengthSelect.value,requested=selected==='full'?audioBuffer.duration:Number(selected),duration=Math.min(audioBuffer.duration,requested),fps=30;
    ...
    const target=new ArrayBufferTarget();const muxer=new Muxer({target,video:{codec:'avc',width,height},audio:{codec:'aac',sampleRate:audioBuffer.sampleRate,numberOfChannels:audioBuffer.numberOfChannels},fastStart:'in-memory'});
    ...
    const videoConfig={codec:'avc1.420028',width,height,bitrate:12000000,framerate:fps,avc:{format:'avc'}};const audioConfig={codec:'mp4a.40.2',sampleRate:audioBuffer.sampleRate,numberOfChannels:audioBuffer.numberOfChannels,bitrate:192000};
    ...
    let previousEnergy=0;const frameCount=Math.ceil(duration*fps);for(let frame=0;frame<frameCount;frame++){const time=frame/fps,snap=offlineSnapshot(audioBuffer,time,previousEnergy);previousEnergy=snap.bass;dataArray.set(snap.timeData);draw(time*1000,true,snap);const videoFrame=new VideoFrame(canvas,{timestamp:Math.round(time*1e6),duration:Math.round(1e6/fps)});videoEncoder.encode(videoFrame,{keyFrame:frame%60===0});videoFrame.close();if(videoEncoder.encodeQueueSize>8)await videoEncoder.flush();if(frame%12===0){const pct=Math.round(frame/frameCount*85);button.innerHTML=`<span>◌</span> RENDERING ${pct}%`;await new Promise(requestAnimationFrame)}if(encodeError)throw encodeError}
    await videoEncoder.flush();button.innerHTML='<span>◌</span> MIXING AUDIO 90%';
    const audioFrames=Math.min(audioBuffer.length,Math.ceil(duration*audioBuffer.sampleRate)),chunkSize=2048;for(let offset=0;offset<audioFrames;offset+=chunkSize){const frames=Math.min(chunkSize,audioFrames-offset),planar=new Float32Array(frames*audioBuffer.numberOfChannels);for(let channel=0;channel<audioBuffer.numberOfChannels;channel++)planar.set(audioBuffer.getChannelData(channel).subarray(offset,offset+frames),channel*frames);const audioData=new AudioData({format:'f32-planar',sampleRate:audioBuffer.sampleRate,numberOfFrames:frames,numberOfChannels:audioBuffer.numberOfChannels,timestamp:Math.round(offset/audioBuffer.sampleRate*1e6),data:planar});audioEncoder.encode(audioData);audioData.close();if(audioEncoder.encodeQueueSize>16)await audioEncoder.flush()}
    await audioEncoder.flush();muxer.finalize();downloadBlob(new Blob([target.buffer],{type:'video/mp4'}),'mp4');$('#exportNote').textContent=`1080p MP4 exported in ${width} × ${height}.`;button.innerHTML='<span>✓</span> EXPORTED';setTimeout(()=>button.innerHTML='<span>↓</span> EXPORT 1080P MP4',1600);
